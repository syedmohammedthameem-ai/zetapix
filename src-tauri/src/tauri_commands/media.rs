...
pub async fn compress_media_batch(
    ...
    for (index, media_item) in media.iter().enumerate() {
        ...
        if let Some(video_config) = &media_item.video_config {
            ...
            let target_reduction = video_config.target_reduction;
            ...
            match ffmpeg_instance
                .compress_video(
                    ...
                    subtitles_config,
                    speed,
                    video_config.chip.as_deref(),
                    video_config.force_target.unwrap_or(false),
                    video_config.target_video_bitrate_bps,
                    video_config.effort.as_deref(),
                )
                .await
            {
                Ok(result) => {
