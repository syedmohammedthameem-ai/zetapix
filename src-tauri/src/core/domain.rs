...
pub struct VideoCompressionConfig {
    ...
    pub quality: u16,
    pub target_reduction: Option<u8>,
    pub dimensions: Option<(f64, f64)>,
    ...
    pub force_target: Option<bool>,
    /// Video bitrate the ratio resolves to. When set it decides the rate
    /// outright: no quantiser, no safeguard, no second-guessing.
    pub target_video_bitrate_bps: Option<u64>,
    /// "fast" runs on whatever hardware exists, "balanced" and "best" run on
    /// the CPU for better quality at the same size.
